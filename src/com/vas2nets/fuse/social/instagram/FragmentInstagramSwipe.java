 ...
 * 
 */
public class FragmentInstagramSwipe extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View fragContent = inflater.inflate(R.layout.fragment_fragment_instagram_swipe,
				container, false);
		
		mViewPager = (ViewPager) fragContent.findViewById(R.id.pagerinstagram);
	...
	private class InnerPagerAdapter extends FragmentPagerAdapter {
		...
		public Fragment getItem(int position) {
			if (position == 0) {
				return new FragmentInstagramHome();
			} else if (position == 1) {
				return new FragmentInstagramFollowers();
			} else if(position == 2){
				return new FragmentInstagramShare();
			}
			return null;
		}
