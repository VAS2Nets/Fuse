 ...
 * 
 */
public class FragmentLinkedInSwipe extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View fragContent = inflater.inflate(R.layout.fragment_fragment_linked_in_swipe,
				container, false);
		
		mViewPager = (ViewPager) fragContent.findViewById(R.id.pagerlinkedin);
		...
		tabHost.addTab(tabHost.newTabSpec("Tab2").setIndicator("Connects").setContent(mFactory));
	...
	private class InnerPagerAdapter extends FragmentPagerAdapter {
		...
		public Fragment getItem(int position) {
			if (position == 0) {
				return new FragmentLinkedIn();
			} else if (position == 1) {
				return new FragmentLinkedInConnect();
			} else if(position == 2){
				return new FragmentKLinkedInShare();
			}
			return null;
		}
